...
## 18. Sealed Interfaces / Sealed Classes

A `sealed` interface (or class) explicitly lists every type allowed to
implement/extend it — nothing else, ever, unless that list is edited.
...
This says: *only* `HotelBookingResponse` and `FlightBookingResponse` may
implement `BookingResponse`. A normal (non-sealed) interface can be
implemented by any class anywhere in the codebase — there's no way to know
the full list of implementations just by looking at the interface itself.

**Why it matters:** it makes a `switch` expression over that type provably
*exhaustive*. The compiler knows the sealed type's permitted subtypes ahead
of time, so it can verify every case is handled without needing a `default`
branch:

```java
BookingResponse response = switch (booking) {
    case HotelBooking hb -> toHotelResponse(hb);
    case FlightBooking fb -> toFlightResponse(fb);
};
```

If a third booking type were ever added later, every `switch` like this one
across the codebase would immediately fail to compile until a case for it
was added — turning "forgot to handle a new case" from a runtime bug into a
compile-time error.

**Where it showed up:** `booking-service`'s `BookingResponse` — the response
type for `GET /bookings/mine`'s unified list (mixed hotel + flight bookings
in one query, one endpoint). Each JSON item only ever has its own real
fields; Jackson adds a `"type"` discriminator (`@JsonTypeInfo`/`@JsonSubTypes`)
so the frontend knows which shape it received. This pairs with `Booking`'s
JOINED JPA inheritance (`HotelBooking`/`FlightBooking` extend it) — the
sealed interface is the DTO-layer equivalent of that same "closed set of two
variants" idea, just applied to the API response shape instead of the
database schema.
