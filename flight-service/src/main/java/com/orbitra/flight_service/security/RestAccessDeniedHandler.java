/*
  ...
  rule (e.g. hasAuthority("PARTNER_FLIGHT")) - that AccessDeniedException is
  thrown inside Spring Security's filter chain, before the request ever
  reaches DispatcherServlet, so GlobalExceptionHandler's @RestControllerAdvice
  cannot see it. This handler exists so a wrong-role/wrong-authority 403 still
  gets the same JSON ErrorResponse shape as every other error in the app,
  instead of Spring Security's blank default body.
*/
package com.orbitra.flight_service.security;
