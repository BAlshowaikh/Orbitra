/*
  ...
  rule (hasRole("TRAVELER") - the only rule this service has) - that
  AccessDeniedException is thrown inside Spring Security's filter chain,
  before the request ever reaches DispatcherServlet, so
  GlobalExceptionHandler's @RestControllerAdvice cannot see it. This handler
  exists so a wrong-role 403 still gets the same JSON ErrorResponse shape as
  every other error in the app, instead of Spring Security's blank default body.
*/
...
public class RestAccessDeniedHandler implements AccessDeniedHandler {
    ...
    // serialization consistent with the rest of the app's responses.
